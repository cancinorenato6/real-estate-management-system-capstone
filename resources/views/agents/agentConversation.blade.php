...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>Conversation about {{ $property ? $property->title : 'General Inquiry' }}</h2>
        <a href="{{ route('agentMessages') }}" class="btn btn-outline-secondary">
            <i class="fas fa-arrow-left me-1"></i> Back to Messages
        </a>
    </div>
    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <h4 class="mb-0"><i class="fas fa-building me-2"></i>{{ $property ? $property->title : 'General Inquiry' }}</h4>
        ...
        <div class="card-body" style="max-height: 500px; overflow-y: auto; display: flex; flex-direction: column-reverse;">
            @if($messages->isEmpty())
                <div class="text-center py-4 text-muted">
                    <i class="fas fa-comment-alt mb-2" style="font-size: 2rem;"></i>
                    <p class="mb-0">No messages yet. Start the conversation below.</p>
                </div>
            @else
                @foreach($messages as $message)
                    <div class="mb-3 {{ $message->sender_type === 'agent' ? 'text-end' : 'text-start' }}">
                        <div class="d-inline-block p-3 rounded {{ $message->sender_type === 'agent' ? 'bg-primary text-white' : 'bg-light' }}" style="max-width: 70%;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch('{{ route('agent.sendMessage') }}', {
                method: 'POST',
                headers: {
...
<div class="container py-4">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <h4 class="mb-0"><i class="fas fa-building me-2"></i>{{ $property->title }}</h4>
            <p class="text-muted mb-0">Client: {{ $client->name }}</p>
        </div>
        <div class="card-body" style="max-height: 500px; overflow-y: auto; display: flex; flex-direction: column-reverse;">
            @if($messages->isEmpty())
...
<div class="container py-4">
    ...
    <div class="card shadow-sm hover-shadow">
        <div class="card-header bg-white">
            <h4 class="mb-0"><i class="fas fa-building me-2"></i>{{ $property->title }}</h4>
            <p class="text-muted mb-0">Client: {{ $client->first_name }}</p>
        </div>
        <div class="card-body" style="max-height: 500px; overflow-y: auto; display: flex; flex-direction: column-reverse;">
            @if($messages->isEmpty())
                <div class="text-center py-4 text-muted">
...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>
            ...
            @if(is_null($property->id))
                General Inquiry
            @else
                Conversation about {{ $property->title }}
            @endif
        </h2>
        <a href="{{ route('agentMessages') }}" class="btn btn-outline-secondary">
            <i class="fas fa-arrow-left me-1"></i> Back to Messages
    ...
    <div class="card shadow-sm hover-shadow">
        <div class="card-header bg-white">
            {{-- @if($property->id == 0) --}}
            @if(is_null($property->id))
                <h4 class="mb-0"><i class="fas fa-comment-alt me-2"></i>General Inquiry</h4>
            @else
                <h4 class="mb-0"><i class="fas fa-building me-2"></i>{{ $property->title }}</h4>
            @endif
            <p class="text-muted mb-0">Client: {{ $client->first_name }}</p>
