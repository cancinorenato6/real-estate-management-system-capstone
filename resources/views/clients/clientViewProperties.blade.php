            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="contactAgentForm">
                        ...
                        <div class="mb-3">
                            <label for="message-text" class="form-label">Message</label>
                            <textarea class="form-control" id="message-text" name="message" rows="5" placeholder="I am interested in this property and would like more information..."></textarea>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Open modal on image click and initialize modal carousel
    document.querySelectorAll('.property-img').forEach((img) => {
        img.addEventListener('click', function() {
            const index = this.getAttribute('data-index');
            const modal = new bootstrap.Modal(document.getElementById('imageModal'));
    ...
    document.querySelector('.favorite-form')?.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const errorData = await response.json();
                console.error('Error:', errorData.error);
                if (response.status === 401) window.location.href = '/clientLogin';
            }
        } catch (error) {
            console.error('Fetch error:', error);
        } finally {
